...
void AdminModeGUI::initAdminModeGUI() {
    ...
    this->coatsListWidget->setStyleSheet(
        "QTableWidget {"
    ...
    this->undoButton->setStyleSheet("QPushButton { background-color: #ADD8E6; }");
    this->redoButton->setStyleSheet("QPushButton { background-color: #ADD8E6; }");
    ...
    buttonsLayout->addWidget(this->goBackButton, 0, 2);
    ...
    row1->addWidget(this->deleteButton);
    row1->addWidget(this->goBackButton);

    QHBoxLayout* row2 = new QHBoxLayout();
    row2->addWidget(this->updatePriceButton);
    row2->addWidget(this->updateQuantityButton);
    row2->addWidget(this->displayButton);

    QHBoxLayout* row3 = new QHBoxLayout();
    row3->addWidget(this->undoButton);
    row3->addWidget(this->redoButton);

    mainLayout->addLayout(row1);
    mainLayout->addLayout(row2);
    mainLayout->addLayout(row3);

    adminLayout->addLayout(mainLayout);
...
void AdminModeGUI::populateList() {
    ...
    vector<TrenchCoat> Coats = this->service.GetRepository();
...
void AdminModeGUI::connectSignalsAndSlots() {
    QObject::connect(this->coatsListWidget, &QListWidget::itemSelectionChanged, [this]() {
        ...
        TrenchCoat c = this->service.GetRepository()[selectedIndex];
    ...
    QObject::connect(this->goBackButton, &QPushButton::clicked, this, &AdminModeGUI::clickGoBack);
    ...
    QObject::connect(this->displayButton, &QPushButton::clicked, this, &AdminModeGUI::goBack);
...
void AdminModeGUI::clickGoBack()
{
    ...
    this->coatsListWidget->clear();
    auto coats = this->service.GetRepository();
    for (const auto& coat : coats) {
        QString itemText = QString::fromStdString(coat.toString());
        QListWidgetItem* item = new QListWidgetItem(itemText);
        this->coatsListWidget->addItem(item);
    }
}

void AdminModeGUI::goBack()
{
